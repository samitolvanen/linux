...
pub(crate) fn mmap(device: &TyrDrmDevice, _file: &TyrDrmFileData, vma: &VmaNew) -> Option<Result> {
...
unsafe extern "C" fn vm_fault_handler(vmf: *mut bindings::vm_fault) -> bindings::vm_fault_t {
    ...
    // SAFETY: `vma` is the active VMA for this fault, and the kernel holds
    // the mmap read lock for the duration of the callback.
