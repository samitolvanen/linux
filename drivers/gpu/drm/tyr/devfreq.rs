...
pub(crate) fn init(
    ...
    *tdev.opp_config.lock() = Some(config_token);
    ...
    // `opp::Table` is not `Unpin` with CONFIG_CPUMASK_OFFSTACK=n, so the guard
    // has no `DerefMut`.
    tdev.devfreq_data.opp_table.lock().as_mut().set(Some(table));

    let cur_freq = core_clk.rate();
