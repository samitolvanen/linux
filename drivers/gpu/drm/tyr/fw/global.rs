...
impl GlobalInterface {
    ...
    ///
    /// Wait failures (timeout, signal, transient I/O error) are
    /// visible in the returned mask rather than in the error path, so
    /// callers can distinguish a stuck slot from a successful
    /// transition and see exactly which sub-requests were honoured.
    ///
    ...
    pub(crate) fn wait_csg_acks(
        ...
        let _ = self.event_wait.wait_interruptible_timeout(timeout_ms, || {
