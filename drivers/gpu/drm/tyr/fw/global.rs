...
/// MCU firmware topology, derived from the firmware control area.
///
/// Used to size preallocations made before the firmware shared-section
/// mutex is taken in [`enable`](EnabledGlobalInterface).
pub(crate) struct McuTopology {
    /// Number of CSG (Command Stream Group) slots advertised by the MCU.
    pub(crate) group_num: u32,
    /// Number of command streams per CSG slot.
...
pub(crate) struct Input {
    ...
    pub(crate) perfcnt_config: u32,
    pub(crate) perfcnt_csg_select: u32,
...
impl GlobalInterface {
    pub(super) fn new(
        shared_section: SharedSectionBacking,
        iomem: Arc<Devres<IoMem>>,
        req_wait: Arc<Wait>,
    ) -> Result<Self> {
    ...
    /// Reads the firmware's [`Control`] structure once the MCU has booted.
    ///
    /// Polls for a non-zero version field for up to 200ms, then returns the
    /// [`Control`] snapshot. Returns `EINVAL` if the firmware reports
    /// version 0 after the timeout.
    fn read_control_after_boot(&self) -> Result<Control> {
    ...
    /// Reads the MCU topology (number of CSG slots and command streams per
    /// CSG) from the firmware's control area.
    pub(crate) fn read_topology(&self) -> Result<McuTopology> {
        let control = self.read_control_after_boot()?;

        let csg_start = constants::CSF_GROUP_CONTROL_OFFSET as usize;
        let csg_control_area = SharedSectionRange {
            shared_section: self.shared_section.clone(),
            start: csg_start,
            end: csg_start + core::mem::size_of::<csg::Control>(),
        };
        let csg_control = csg::Control::read(&csg_control_area)?;

        Ok(McuTopology {
            group_num: control.group_num,
            stream_num: csg_control.stream_num,
        })
    }
    ...
    pub(crate) fn enable(
        ...
        if streams_per_csg.len() != control.group_num as usize {
            pr_err!(
                "Expected {} preallocated stream vectors\n",
                control.group_num
            );
            return Err(EINVAL);
        }
        ...
        // Zero req/doorbell fields so toggle_reqs starts from a clean state.
        // Input::read() preserves whatever the FW left in shared memory, which
        // can leave doorbell_req non-zero and cause the first CSG doorbell
        // toggle to be a no-op (FW sees no transition).
        input.req = 0;
        input.doorbell_req = 0;
    ...
    /// Process pending global IRQ events. ACKs GLB_IDLE so the FW
    /// can report further idle events.
    pub(crate) fn process_global_irq(&mut self) -> Result {
        let glb = match &self.state {
        ...
        if pending_evts & GLB_IDLE != 0 {
            /* ACK GLB_IDLE so FW can report further idle events. */
            let req = RequestField::new(
                &glb.input_area,
                core::mem::offset_of!(Input, req),
                &glb.output_area,
    ...
    pub(crate) fn arm_watchdog(&self, tdev: &TyrDrmDevice) -> Result {
        tdev.reset_wq
            .enqueue_delayed::<_, 0>(ARef::from(tdev), PING_INTERVAL_MS as usize)
            .map_err(|_| EINVAL)
    }
    ...
    pub(super) fn shared_range(&mut self, mcu_va: u64, size: usize) -> Result<SharedSectionRange> {
        let shared_mem_start = self.shared_section.lock().va_start();
        let shared_mem_end = self.shared_section.lock().va_end();
...
impl_has_delayed_work! {
    impl HasDelayedWork<TyrDrmDevice, 0> for TyrDrmDeviceData {
        self.ping_work
    }
}

impl WorkItem<0> for TyrDrmDeviceData {
    type Pointer = ARef<TyrDrmDevice>;

    fn run(this: Self::Pointer) {
        let data: &TyrDrmDeviceData = &*this;
        let res = data.fw.with_locked_global_iface(|glb| {
            glb.ping()?;
            data.reset_wq
