        ...
        if devquery.pointer == 0 {
            match devquery.type_ {
                ...
                uapi::drm_panthor_dev_query_type_DRM_PANTHOR_DEV_QUERY_CSIF_INFO => {
                    devquery.size = core::mem::size_of::<CsifInfo>() as u32;
                    Ok(0)
                }
            ...
            match devquery.type_ {
                ...
                uapi::drm_panthor_dev_query_type_DRM_PANTHOR_DEV_QUERY_CSIF_INFO => {
                    let csif = reg_data.csif_info.lock();
    ...
    pub(crate) fn heap_create(
...

        {
            ...
            if guard.get(vm_id).is_none() {
                drop(guard);
                let pool = Arc::new(heap::Pool::create(ddev, reg_data, vm.clone())?, GFP_KERNEL)?;
        ...
        let created_context = {
            ...
            pool.create_heap_context(ddev, reg_data, args)?
        };
...
impl RawQueueSubmit {
    ...
    fn capture(self) -> Result<QueueSubmit> {
        ...
        if stream_size != 0 {
            ...
            let mut reader =
                UserSlice::new(UserPtr::from_addr(self.0.stream_addr as usize), stream_size)
                    .reader();
