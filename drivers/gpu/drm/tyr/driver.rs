...
/// and the [`KernelBo`](crate::gem::KernelBo)s that enqueue deferred
/// drops on it. The wrapped [`Queue`](kernel::workqueue::Queue) is
...
impl platform::Driver for TyrPlatformDriverData {
    ...
    fn probe(
        ...
        // find a valid, correctly-typed pointer. The framework's post-probe
        // set_drvdata stores another *mut TyrPlatformDriverData (boxed from
        // the probe return value), so the slot type stays consistent across
        // the overwrite and the re-stamped type id remains correct.
