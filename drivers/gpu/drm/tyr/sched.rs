...
impl Scheduler {
    pub(crate) fn init(tdev: &TyrDrmDevice) -> Result<Self> {
        ...
        let wq = workqueue::system_unbound();
    ...
    pub(crate) fn bind_group(
        ...
        let as_nr = group.vm.address_space();
    ...
    fn find_free_slot(&mut self, tdev: &TyrDrmDevice) -> Result<usize> {
        let csg_idx = self
            .csg_slots
            .iter()
    ...
    fn evict_idle_group(&mut self, tdev: &TyrDrmDevice) -> Result<usize> {
        let mut idx = None;
        for (i, slot) in self
            .csg_slots
            .iter()
    ...
    #[expect(dead_code)]
    pub(crate) fn issue_dummy_instr(&mut self, group: Arc<Group>, tdev: &TyrDrmDevice) -> Result {
        self.bind(tdev, group.clone())?;

        let iomem = tdev.iomem.clone();

        let flags = crate::vm::VmMapFlags::from(crate::vm::VmFlag::Noexec)
            | crate::vm::VmMapFlags::from(crate::vm::VmFlag::Uncached);

        let mut debug_gem = gem::new_kernel_object(tdev, &group.vm, SZ_4K, flags)?;
