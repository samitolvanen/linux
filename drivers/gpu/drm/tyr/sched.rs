...
impl Scheduler {
        ...
        for entry in snapshot.iter() {
            ...
            for (queue_idx, queue) in group.queues.iter().enumerate() {
                match group.read_syncobj(queue_idx) {
                    Ok(syncobj) => {
                        queue.complete_submit_fences(group, syncobj.seqno, syncobj.status)
                    }
                    Err(err) => pr_err!(
    ...
    /// dependency.
    ///
    /// The caller resolves each group's heap pool and runs the dump
    ...
    /// `collect_heap_pools_for_trace`. Groups whose VM's `heap_pool`
    /// mutex is contended are silently skipped via
    /// `Group::try_get_heap_pool`; duplicates that arise from a group
    /// appearing on both a priority list and a CSG slot are filtered by
    /// `Arc::ptr_eq` on the pool.
    ///
    /// Downstream-only debug aid; not for upstream.
    ...
    /// pool is resolved, and both the VM's `heap_pool` mutex and the
    ...
    /// other than `group_uid`, or the VM's `heap_pool` mutex is
    /// contended.
    ///
    /// Downstream-only debug aid; not for upstream.
