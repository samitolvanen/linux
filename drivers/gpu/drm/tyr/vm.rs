...
pub(crate) struct VmExec {
    ...
    va_range: Range<u64>,
    /// Ships the gpuvm references to the cleanup workqueue and keeps the
    /// device alive until they are released. `None` for the firmware VM,
    /// which the device owns.
    handoff: Option<cleanup::Handoff<GpuVmRelease, ARef<TyrDrmDevice>>>,
...
pub(crate) struct Vm {
    ...
    /// blocked on under the scheduler mutex.
    ///
    ...
    /// Keeps the device alive until every field above has dropped, including
    /// the final put of `root_gem`. `None` for the firmware VM, which the
    /// device owns.
    ddev: Option<ARef<TyrDrmDevice>>,
}
...
/// Owner of a `Vm`.
enum VmOwner {
    /// The device, for the firmware VM.
    Firmware,
    /// A file. The VM holds this device reference until its deferred drops
    /// have run.
...
impl Vm {
    ...
    fn new_with_ranges<Ctx: DeviceContext>(
        ...
        let (fw, device_ref) = match owner {
            VmOwner::Firmware => (true, None),
            VmOwner::User(dev) => (false, Some(dev)),
        };
        ...
        let handoff = device_ref.clone().map(cleanup::Handoff::new).transpose()?;
    ...
    /// Non-blocking variant of `heap_pool`. Returns `None` when the mutex
    /// is contended, so unlike `heap_pool` it can be called under the
    /// scheduler mutex.
    pub(crate) fn try_heap_pool(&self) -> Option<Arc<heap::Pool>> {
        match &*self.heap_pool.try_lock()? {
            HeapPoolState::Live(pool) => Some(pool.clone()),
            HeapPoolState::Empty | HeapPoolState::Destroyed => None,
        }
    }
...
impl DriverGpuVm for GpuVmData {
    ...
    fn sm_step_map<'op>(
        ...
        for &(mut paddr, mut sgt_entry_length) in map_sgt.iter() {
            ...
            let segment_mapped =
                match pt_map(context.page_table, context.vm_id, iova, paddr, len, prot) {
                    ...
                    Err(e) => {
                        ...
                        if total_mapped > 0 {
                            pt_unmap(context.page_table, start_iova..(start_iova + total_mapped))
                                .ok();
                        }
