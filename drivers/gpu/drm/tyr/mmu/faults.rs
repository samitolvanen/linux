...
pub(crate) fn access_type_name(fault_status: u32) -> &'static str {
    let access_bits = (fault_status >> 8) & 0x3;
    match access_bits {
        0 => "ATOMIC",
        1 => "EXECUTE",
        2 => "READ",
...
pub(crate) fn decode_faults(mut status: u32, iomem: &Devres<IoMem>) -> Result {
    ...
    while status != 0 {
        ...
        let exception_type: u32 = fault_status_raw & 0xff;
        let access_type: u32 = (fault_status_raw >> 8) & 0x3;
        let source_id: u32 = fault_status_raw >> 16;
