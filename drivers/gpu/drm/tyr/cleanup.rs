...
///
/// `T` is the value that was not queued.
...
// SAFETY: `T: Send`, so the value may cross to the worker. The pointer is
// turned back into a `KBox` exactly once: by the worker when the item was
// queued, and by `try_spawn_owned` otherwise. The two never alias it.
