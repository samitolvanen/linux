...
pub(crate) mod gpu_control {
    ...
    register! {
        ...
        ///
        /// Bit 24 has two meanings. Cores with a custom ASN hash use it as the
        /// enable bit for `GPU_ASN_HASH`. On older cores it is the low bit of the
        /// hash function index. Only one of the two fields may be written on a
        /// given core.
        pub(crate) L2_CONFIG(u32) @ 0x48 {
            /// Requested cache size.
            23:16   cache_size;
            /// Enables the custom ASN hash held in `GPU_ASN_HASH`, a 1-bit boolean
            /// flag.
        ...
        /// Custom ASN hash values, programmed before the L2 block powers up when
        /// `L2_CONFIG::asn_hash_enable` is set.
...
pub(crate) mod pwr_control {
    ...
    register! {
        /// IRQ sources raw status.
        /// Writing to this register forces bits on, but does not clear them.
        pub(crate) PWR_IRQ_RAWSTAT(u32) @ 0x800 {
        ...
        /// IRQ sources to clear. Write only.
        pub(crate) PWR_IRQ_CLEAR(u32) @ 0x804 {
            /// Clear the POWER_CHANGED_SINGLE interrupt, a 1-bit boolean flag.
            0:0     power_changed_single => bool;
            /// Clear the POWER_CHANGED_ALL interrupt, a 1-bit boolean flag.
            1:1     power_changed_all => bool;
        ...
        /// IRQ sources enabled.
        pub(crate) PWR_IRQ_MASK(u32) @ 0x808 {
            /// Enable the POWER_CHANGED_SINGLE interrupt, a 1-bit boolean flag.
            0:0     power_changed_single => bool;
            /// Enable the POWER_CHANGED_ALL interrupt, a 1-bit boolean flag.
            1:1     power_changed_all => bool;
        ...
        /// IRQ status for enabled sources. Read only.
        pub(crate) PWR_IRQ_STATUS(u32) @ 0x80c {
            /// POWER_CHANGED_SINGLE interrupt status, a 1-bit boolean flag.
            0:0     power_changed_single => bool;
            /// POWER_CHANGED_ALL interrupt status, a 1-bit boolean flag.
            1:1     power_changed_all => bool;
...
pub(crate) mod mmu_control {
    ...
    pub(crate) mod mmu_as_control {
        ...
        impl MEMATTR {
            ...
            /// - device/write-through/non-cacheable maps to GPU `NonCacheable`
            /// - write-back maps to GPU `WriteBack` (preserving inner allocation hints)
            ///
            /// `coherent` selects the shareability domain of the write-back
            /// bytes.
