...
pub(crate) struct Group {
    ...
    /// Zero until the group is inserted into the pool.
    handle: AtomicU64,
...
impl Group {
    ...
    /// up after `can_run()` returns false). Repeat calls are dropped
    /// via the `term_scheduled` latch in `GroupInner`.
    ...
    /// subsequent re-evaluations skip the gpuvm walk. The cache is
    /// keyed by its own `(gpu_va, sync64)` and the snapshot's live
    /// fields can move on without the cache being touched; this
    /// helper falls through to a rebuild when the keys no longer
    /// match.
    ///
    ...
    pub(crate) fn eval_syncwait(&self, queue_idx: usize) -> Result<bool> {
        ...
        {
            if syncwait.sync64 {
                syncs::SyncObj64b::read(&cached.bo, cached.offset)?.seqno
            } else {
                u64::from(syncs::SyncObj32b::read(&cached.bo, cached.offset)?.seqno)
            }
        } else {
    ...
    /// Non-blocking variant of [`Self::get_heap_pool`] for use from
    /// dma-fence signalling sections. Returns `None` both when no
    /// pool is bound and when the heap_pool mutex is contended.
    pub(super) fn try_get_heap_pool(&self) -> Option<Arc<heap::Pool>> {
        self.heap_pool.try_lock()?.clone()
    }
...
impl deps::BatchOps for SubmitOps {
    ...
    fn prepare(
        ...
        let wrapped = if has_stream {
            ...
            job.build_wrapped_stream(&self.group, sync_va, job_counter)?
        } else {
            KVec::new()
        };
    ...
    /// Returns the job's own submit fence for a command-stream job, or
    /// the adopted prior command-stream fence for a stream-less job.
...
impl Pool {
    ...
    pub(crate) fn create_group(
        ...
        // The pool entry published above becomes unreachable if the group
        // never joins the scheduler, so take it back on failure. The group
        // was never added, so no tick can have bound it.
        if let Err(e) = ddev.with_locked_scheduler(|sched| sched.add_group(group.clone())) {
            let _ = self.0.remove(handle);
            return Err(e);
        }
