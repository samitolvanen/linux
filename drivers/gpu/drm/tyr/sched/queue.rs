...
impl QueueData {
    ...
    fn complete_pending_fences_up_to(&self, group: &Group, up_to_seqno: u64, raw_status: u32) {
        let mut drained_count: u32 = 0;
        let mut last_done_seqno: u64 = 0;

        loop {
            let (done_seqno, profiling_slot, profiling_mask, fence) = {
                let mut pending = self.pending_submit_fences.lock();
                let head = pending.head;
                let Some(entry) = pending.vec.get_mut(head) else {
                    Self::maybe_truncate_pending(&mut pending);
                ...
                let fence = entry.fence.take();
                pending.head = head + 1;
                (done_seqno, profiling_slot, profiling_mask, fence)
            };
    ...
    pub(in crate::sched) fn complete_submit_fences(
        &self,
        group: &Group,
        syncobj_seqno: u64,
        syncobj_status: u32,
    ) {
        self.complete_pending_fences_up_to(group, syncobj_seqno, syncobj_status);
    }
...
impl StageOps<TyrQueueOps> for QueueCompletionStage {
    fn process(&self, ctx: &StageContext<'_, TyrQueueOps>) -> StageAdvance {
        ...
        match ctx.job.group.read_syncobj(ctx.job.queue_index) {
            Ok(syncobj) => {
                self.data
                    .complete_submit_fences(&ctx.job.group, syncobj.seqno, syncobj.status)
            }
            Err(err) => {
