...
pub(crate) fn tick_step(tdev: &ARef<TyrDrmDevice>, fw: &Firmware<'_>) -> Result {
    ...
    let Some(_active) = tdev.sched_pm_get_if_active() else {
        ...
        // than spinning against the resume's own immediate tick. The scheduler
        // flag covers the force-suspend window `pm_suspended` misses.
        if !tdev.sched_suspended.load(Relaxed) {
            if resume_failed && resume_pending {
                Scheduler::request_resume_retry(tdev);
            } else if (resume_pending || kick_pending) && !tdev.pm_suspended() {
