...
impl Instr {
    ...
    fn sync_add64(addr_reg: u64, val_reg: u64) -> u64 {
        // No scoreboard wait, because a prior `WAIT` already drained.
...
/// submit that runs no GPU work. Only the dependency and signal
/// `SyncOp`s carried alongside the piece are honored.
#[derive(Copy, Clone)]
struct StreamPiece {
...
impl PreparedQueueSubmit {
    pub(crate) fn commit(self, group: &Group) -> Result<ARef<PublicDmaFence>> {
        ...
        // `SYNC_ADD64`. Do not seed it from the CPU here, because a
        // back-to-back commit on the same queue would otherwise
        // overwrite GPU progress for the previous Job that has not yet
        // retired.
...
impl Job {
    ...
    fn build_wrapped_stream(&self, csif: &CsifInfo, sync_va: u64) -> Result<KVec<u8>> {
        const INSTR_BYTES: usize = 8;
        const INSTRS_PER_WRAPPER: usize = 11;
        const WRAPPER_BYTES: usize = INSTR_BYTES * INSTRS_PER_WRAPPER;

        // `cs_reg_count` is the total CS register count for this chip.
        // The firmware reserves the top `UNPRESERVED_CS_REG_COUNT` of
        // them for the kernel-side wrapper.
        let addr_reg = u64::from(
            csif.cs_reg_count
                .checked_sub(UNPRESERVED_CS_REG_COUNT)
                .ok_or(EINVAL)?,
        );
        let val_reg = addr_reg + 2;
        let top_sb = csif.scoreboard_slot_count.checked_sub(1).ok_or(EINVAL)?;
        ...
        // `GFP_KERNEL` anyway, since this runs at prepare time, outside
        // any dma-fence signalling section.
    ...
    pub(crate) fn prepare(
        ...
        let wrapped = if has_stream {
            ...
            self.build_wrapped_stream(csif, sync_va)?
        } else {
            KVec::new()
        };
