...
pub(crate) fn pwr_irq_enable(io: &IoMem) {
    io.write_reg(PWR_IRQ_CLEAR::from_raw(u32::MAX));
    io.write_reg(PWR_IRQ_MASK::from_raw(pwr_interrupts_mask()));
}

/// Masks all PWR IRQ sources.
pub(crate) fn pwr_irq_disable(io: &IoMem) {
    io.write_reg(PWR_IRQ_MASK::from_raw(0));
}
...
impl TyrIrqTrait for PwrIrq {
    fn read_status(&self, dev: &Device<Bound>) -> u32 {
        self.iomem
            .access(dev)
            .map(|io| io.read(PWR_IRQ_STATUS).into_raw())
            .unwrap_or_default()
    }
    ...
    fn reenable(&self, dev: &Device<Bound>) {
        if let Ok(io) = self.iomem.access(dev) {
            io.write_reg(PWR_IRQ_MASK::from_raw(self.mask));
        }
    }
    ...
    fn read_raw_status(&self, dev: &Device<Bound>) -> u32 {
        self.iomem
            .access(dev)
            .map(|io| io.read(PWR_IRQ_RAWSTAT).into_raw())
            .unwrap_or_default()
    }

    fn clear_status(&self, dev: &Device<Bound>, status: u32) {
        if let Ok(io) = self.iomem.access(dev) {
            io.write_reg(PWR_IRQ_CLEAR::from_raw(status));
        }
    }
    ...
    fn handle(&self, tdev: &TyrDrmDevice, status: u32) {
        let status = PWR_IRQ_STATUS::from_raw(status);
...
///
/// Completion is polled from `PWR_IRQ_RAWSTAT`, which latches the bit
/// regardless of `PWR_IRQ_MASK`, so `reset_completed` stays masked.
fn reset(dev: &Device, iomem: &Devres<IoMem>, command: PwrCommand) -> Result {
    {
        ...
        io.write_reg(PWR_IRQ_CLEAR::zeroed().with_reset_completed(true));
