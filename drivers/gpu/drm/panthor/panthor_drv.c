				      ...
				      struct drm_file *file)
{
	...
	for (u32 i = 0; i < args->queue_submits.count; i++) {
		...
		trace_panthor_ioctl_queue_submit(args->group_handle,
						 qsubmit->queue_index,
						 qsubmit->stream_addr,
						 qsubmit->stream_size,
...
out_dev_exit:
	drm_dev_exit(cookie);
	trace_panthor_ioctl_group_submit(args->group_handle,
					 args->queue_submits.count, ret);
	return ret;
}
...
out:
	trace_panthor_ioctl_group_create(args->vm_id, args->queues.count,
					 args->priority, args->compute_core_mask,
					 args->fragment_core_mask,
					 args->tiler_core_mask,
					 args->group_handle, ret);
	kvfree(queue_args);
	return ret;
}
...
out_put_vm:
	panthor_vm_put(vm);
	trace_panthor_ioctl_tiler_heap_create(args->vm_id,
					      args->initial_chunk_count,
					      args->chunk_size,
					      args->max_chunks,
					      args->target_in_flight,
