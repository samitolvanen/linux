...
TRACE_EVENT(panthor_fw_csg_activate_config,
	TP_PROTO(u32 csg_id, u32 ep_req_raw, u32 config_raw, u64 allow_compute,
		 u64 allow_fragment, u32 allow_other, u32 ack_irq_mask),
	TP_ARGS(csg_id, ep_req_raw, config_raw, allow_compute, allow_fragment,
		allow_other, ack_irq_mask),
	TP_STRUCT__entry(
		__field(u32, csg_id)
	...
	TP_fast_assign(
		...
		__entry->ack_irq_mask = ack_irq_mask;
	),
	TP_printk("csg=%u ep_req=0x%08x(compute=%u fragment=%u tiler=%u prio=%u) config=0x%08x(jasid=%u) allow_compute=0x%llx allow_fragment=0x%llx allow_other=0x%08x ack_irq_mask=0x%08x",
		  ...
		  __entry->allow_other, __entry->ack_irq_mask)
);
...
TRACE_EVENT(panthor_fw_cs_activate_inputs,
	...
	TP_ARGS(csg_id, cs_id, ringbuf_base, ringbuf_size, ringbuf_input_va,
		ringbuf_output_va, config, ack_irq_mask),
	TP_STRUCT__entry(
		__field(u32, csg_id)
		__field(u32, cs_id)
	...
	TP_printk("csg=%u cs=%u ringbuf_base=0x%llx size=0x%x ringbuf_input=0x%llx ringbuf_output=0x%llx config=0x%08x(priority=%u doorbell=%u) ack_irq_mask=0x%08x",
		  ...
		  __entry->ack_irq_mask)
);

TRACE_EVENT(panthor_fw_cs_heap_realloc,
	TP_PROTO(u32 csg_id, u32 cs_id, u64 group_id, u64 new_chunk_va),
	TP_ARGS(csg_id, cs_id, group_id, new_chunk_va),
	TP_STRUCT__entry(
		__field(u32, csg_id)
		__field(u32, cs_id)
		__field(u64, group_id)
		__field(u64, new_chunk_va)
	),
	TP_fast_assign(
		__entry->csg_id = csg_id;
		__entry->cs_id = cs_id;
		__entry->group_id = group_id;
		__entry->new_chunk_va = new_chunk_va;
	),
	TP_printk("csg=%u cs=%u group=%llu heap_start=heap_end=0x%llx",
		  __entry->csg_id, __entry->cs_id, __entry->group_id,
		  __entry->new_chunk_va)
);

TRACE_EVENT(panthor_fw_csg_cs_irq_ack,
	TP_PROTO(u32 csg_id, u64 group_id, u32 cs_irq_ack),
	TP_ARGS(csg_id, group_id, cs_irq_ack),
	TP_STRUCT__entry(
		__field(u32, csg_id)
		__field(u64, group_id)
		__field(u32, cs_irq_ack)
	),
	TP_fast_assign(
		__entry->csg_id = csg_id;
		__entry->group_id = group_id;
		__entry->cs_irq_ack = cs_irq_ack;
	),
	TP_printk("csg=%u group=%llu cs_irq_ack=0x%08x",
		  __entry->csg_id, __entry->group_id, __entry->cs_irq_ack)
);

TRACE_EVENT(panthor_fw_glb_input_init,
	TP_PROTO(u64 core_en_mask, u32 poweroff_timer, u32 progress_timer,
		 u32 idle_timer, u32 ack_irq_mask),
	TP_ARGS(core_en_mask, poweroff_timer, progress_timer, idle_timer,
		ack_irq_mask),
	TP_STRUCT__entry(
		__field(u64, core_en_mask)
		__field(u32, poweroff_timer)
		__field(u32, progress_timer)
		__field(u32, idle_timer)
		__field(u32, ack_irq_mask)
	),
	TP_fast_assign(
		__entry->core_en_mask = core_en_mask;
		__entry->poweroff_timer = poweroff_timer;
		__entry->progress_timer = progress_timer;
		__entry->idle_timer = idle_timer;
		__entry->ack_irq_mask = ack_irq_mask;
	),
	TP_printk("core_en_mask=0x%016llx poweroff_timer=0x%08x progress_timer=0x%08x idle_timer=0x%08x ack_irq_mask=0x%08x",
		  __entry->core_en_mask, __entry->poweroff_timer,
		  __entry->progress_timer, __entry->idle_timer,
		  __entry->ack_irq_mask)
);

TRACE_EVENT(panthor_fw_section_init,
	TP_PROTO(const char *name, u64 va, u64 size, u64 init_size, u32 flags),
	TP_ARGS(name, va, size, init_size, flags),
	TP_STRUCT__entry(
		__string(name, name ? name : "(unnamed)")
		__field(u64, va)
		__field(u64, size)
		__field(u64, init_size)
		__field(u32, flags)
	),
	TP_fast_assign(
		__assign_str(name);
		__entry->va = va;
		__entry->size = size;
		__entry->init_size = init_size;
		__entry->flags = flags;
	),
	TP_printk("name=%s va=0x%llx size=0x%llx init_size=0x%llx flags=0x%08x",
		  __get_str(name), __entry->va, __entry->size,
		  __entry->init_size, __entry->flags)
);

TRACE_EVENT(panthor_fw_queue_iface_init,
	TP_PROTO(u64 va, u64 size, u32 input_fw_va, u32 output_fw_va),
	TP_ARGS(va, size, input_fw_va, output_fw_va),
	TP_STRUCT__entry(
		__field(u64, va)
		__field(u64, size)
		__field(u32, input_fw_va)
		__field(u32, output_fw_va)
	),
	TP_fast_assign(
		__entry->va = va;
		__entry->size = size;
		__entry->input_fw_va = input_fw_va;
		__entry->output_fw_va = output_fw_va;
	),
	TP_printk("va=0x%llx size=0x%llx input_fw_va=0x%08x output_fw_va=0x%08x",
		  __entry->va, __entry->size, __entry->input_fw_va,
		  __entry->output_fw_va)
);

TRACE_EVENT(panthor_kernel_bo_create,
	TP_PROTO(const char *name, u64 va, u64 size, u32 bo_flags,
		 u32 vm_map_flags, bool fw_vm, int result),
	TP_ARGS(name, va, size, bo_flags, vm_map_flags, fw_vm, result),
	TP_STRUCT__entry(
		__string(name, name ? name : "(unnamed)")
		__field(u64, va)
		__field(u64, size)
		__field(u32, bo_flags)
		__field(u32, vm_map_flags)
		__field(bool, fw_vm)
		__field(int, result)
	),
	TP_fast_assign(
		...
		__entry->va = va;
		__entry->size = size;
		__entry->bo_flags = bo_flags;
		__entry->vm_map_flags = vm_map_flags;
		__entry->fw_vm = fw_vm;
		__entry->result = result;
	),
	TP_printk("name=%s va=0x%llx size=0x%llx bo_flags=0x%08x vm_map_flags=0x%08x(%s) fw_vm=%u result=%d",
		  __get_str(name), __entry->va, __entry->size,
		  __entry->bo_flags, __entry->vm_map_flags,
		  __print_flags(__entry->vm_map_flags, "|", PANTHOR_VM_MAP_FLAGS),
		  __entry->fw_vm, __entry->result)
);

TRACE_EVENT(panthor_heap_chunk_init,
	TP_PROTO(u64 va, u64 chunk_size, u64 next),
	TP_ARGS(va, chunk_size, next),
	TP_STRUCT__entry(
		__field(u64, va)
		__field(u64, chunk_size)
		__field(u64, next)
	),
	TP_fast_assign(
		__entry->va = va;
	...
	TP_printk("va=0x%llx chunk_size=0x%llx next=0x%llx",
		  __entry->va, __entry->chunk_size, __entry->next)
);

TRACE_EVENT(panthor_heap_ctx_init,
	TP_PROTO(u32 heap_id, u64 va, u64 stride),
	TP_ARGS(heap_id, va, stride),
	TP_STRUCT__entry(
		__field(u32, heap_id)
		__field(u64, va)
		__field(u64, stride)
	...
	TP_fast_assign(
		__entry->heap_id = heap_id;
		__entry->va = va;
		__entry->stride = stride;
	),
	TP_printk("heap=%u va=0x%llx stride=0x%llx",
		  __entry->heap_id, __entry->va, __entry->stride)
);

TRACE_EVENT(panthor_group_syncobjs_init,
	TP_PROTO(u64 group_id, u64 va, u64 size),
	TP_ARGS(group_id, va, size),
	TP_STRUCT__entry(
		__field(u64, group_id)
		__field(u64, va)
		__field(u64, size)
	...
	TP_printk("group=%llu va=0x%llx size=0x%llx",
		  __entry->group_id, __entry->va, __entry->size)
);

TRACE_EVENT(panthor_group_syncobj_reset,
	TP_PROTO(u64 group_id, u32 queue_idx, u64 va, u32 status, u64 seqno),
	TP_ARGS(group_id, queue_idx, va, status, seqno),
	TP_STRUCT__entry(
		__field(u64, group_id)
	...
	TP_printk("group=%llu queue=%u va=0x%llx status=0x%08x seqno=%llu",
		  __entry->group_id, __entry->queue_idx, __entry->va,
		  __entry->status, __entry->seqno)
);

TRACE_EVENT(panthor_ioctl_group_create,
	TP_PROTO(u32 vm_id, u32 queue_count, u8 priority, u64 compute_core_mask,
		 u64 fragment_core_mask, u64 tiler_core_mask, u32 group_handle,
		 int result),
	TP_ARGS(vm_id, queue_count, priority, compute_core_mask,
		fragment_core_mask, tiler_core_mask, group_handle, result),
	TP_STRUCT__entry(
		__field(u32, vm_id)
		__field(u32, queue_count)
		__field(u8, priority)
		__field(u64, compute_core_mask)
		__field(u64, fragment_core_mask)
		__field(u64, tiler_core_mask)
	...
	TP_fast_assign(
		...
		__entry->compute_core_mask = compute_core_mask;
		__entry->fragment_core_mask = fragment_core_mask;
		__entry->tiler_core_mask = tiler_core_mask;
		__entry->group_handle = group_handle;
		__entry->result = result;
	),
	TP_printk("vm=%u queues=%u priority=%u compute=0x%llx fragment=0x%llx tiler=0x%llx handle=%u result=%d",
		  __entry->vm_id, __entry->queue_count, __entry->priority,
		  __entry->compute_core_mask, __entry->fragment_core_mask,
		  __entry->tiler_core_mask, __entry->group_handle,
		  __entry->result)
);

TRACE_EVENT(panthor_ioctl_group_submit,
	TP_PROTO(u32 group_handle, u32 queue_count, int result),
	TP_ARGS(group_handle, queue_count, result),
	TP_STRUCT__entry(
		__field(u32, group_handle)
		__field(u32, queue_count)
		__field(int, result)
	...
	TP_printk("handle=%u queues=%u result=%d",
		  __entry->group_handle, __entry->queue_count, __entry->result)
);

TRACE_EVENT(panthor_ioctl_queue_submit,
	TP_PROTO(u32 group_handle, u32 queue_index, u64 stream_addr,
		 u32 stream_size, u32 latest_flush),
	TP_ARGS(group_handle, queue_index, stream_addr, stream_size,
		latest_flush),
	TP_STRUCT__entry(
		__field(u32, group_handle)
		__field(u32, queue_index)
		__field(u64, stream_addr)
		__field(u32, stream_size)
		__field(u32, latest_flush)
	),
	TP_fast_assign(
		__entry->group_handle = group_handle;
		__entry->queue_index = queue_index;
		__entry->stream_addr = stream_addr;
		__entry->stream_size = stream_size;
		__entry->latest_flush = latest_flush;
	),
	TP_printk("handle=%u queue=%u stream_addr=0x%llx stream_size=0x%x latest_flush=0x%08x",
		  __entry->group_handle, __entry->queue_index,
		  __entry->stream_addr, __entry->stream_size,
		  __entry->latest_flush)
);

TRACE_EVENT(panthor_ioctl_tiler_heap_create,
	TP_PROTO(u32 vm_id, u32 initial_chunk_count, u32 chunk_size,
		 u32 max_chunks, u32 target_in_flight, u32 handle,
		 u64 heap_ctx_gpu_va, u64 first_chunk_gpu_va, int result),
	TP_ARGS(vm_id, initial_chunk_count, chunk_size, max_chunks,
		target_in_flight, handle, heap_ctx_gpu_va, first_chunk_gpu_va,
		result),
	TP_STRUCT__entry(
		__field(u32, vm_id)
		__field(u32, initial_chunk_count)
		__field(u32, chunk_size)
		__field(u32, max_chunks)
		__field(u32, target_in_flight)
		__field(u32, handle)
		__field(u64, heap_ctx_gpu_va)
		__field(u64, first_chunk_gpu_va)
		__field(int, result)
	),
	TP_fast_assign(
		__entry->vm_id = vm_id;
		__entry->initial_chunk_count = initial_chunk_count;
		__entry->chunk_size = chunk_size;
		__entry->max_chunks = max_chunks;
		__entry->target_in_flight = target_in_flight;
		__entry->handle = handle;
		__entry->heap_ctx_gpu_va = heap_ctx_gpu_va;
		__entry->first_chunk_gpu_va = first_chunk_gpu_va;
		__entry->result = result;
	),
	TP_printk("vm=%u initial_chunks=%u chunk_size=0x%x max_chunks=%u target_in_flight=%u handle=0x%08x heap_ctx=0x%llx first_chunk=0x%llx result=%d",
		  __entry->vm_id, __entry->initial_chunk_count,
		  __entry->chunk_size, __entry->max_chunks,
		  __entry->target_in_flight, __entry->handle,
		  __entry->heap_ctx_gpu_va, __entry->first_chunk_gpu_va,
		  __entry->result)
);
 ...
 * kernel vmap of the pool's heap-context BO. Mirrors Tyr's
 * tyr_heap_context_dump field-for-field so the two drivers can be diffed
 * directly. For the periodic dump path, `group` carries the owning VM's
 * trace id and `cs` is 0; for fault-triggered dumps the call site can
 * supply real group/cs identifiers. Downstream debug aid only.
 */
TRACE_EVENT(panthor_heap_context_dump,
	TP_PROTO(u64 group, u32 cs, u32 heap_index,
		 u64 heap_context_va, u32 chunk_count, const u8 *content),
	TP_ARGS(group, cs, heap_index, heap_context_va,
		chunk_count, content),
	TP_STRUCT__entry(
		__field(u64, group)
		__field(u32, cs)
	...
	TP_printk("group=%llu cs=%u heap=%u va=0x%llx chunks=%u content=%s",
		  __entry->group, __entry->cs, __entry->heap_index,
 ...
 * Mirrors Tyr's tyr_heap_chunk_dump. `chunk_index` is the chunk's
 * position within its heap's chunk chain. Downstream debug aid only.
 */
TRACE_EVENT(panthor_heap_chunk_dump,
	TP_PROTO(u64 group, u32 cs, u32 heap_index, u32 chunk_index,
		 u64 chunk_va, const u8 *header),
	TP_ARGS(group, cs, heap_index, chunk_index, chunk_va, header),
	TP_STRUCT__entry(
		__field(u64, group)
		__field(u32, cs)
		__field(u32, heap_index)
