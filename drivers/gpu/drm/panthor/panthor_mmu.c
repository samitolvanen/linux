 ...
 * @vm: VM to inspect.
 *
 * It is lock-free, using the RCU-based unlocked dma_resv iterator, so it is
 * safe to call from any context, including a dma-fence signalling section.
...
/*
 * Walk every live VM and dump its heap pool to the
 * panthor_heap_context_dump / panthor_heap_chunk_dump tracepoints. The
 * pool reference is taken under vm->heaps.lock so the pool can't be
 * destroyed while we dump it, but the walk itself is best-effort -- any
 * VM whose pool we can't reach without blocking is silently skipped.
 *
 * Downstream debug aid only.
 */
void panthor_mmu_dump_heap_pools_for_trace(struct panthor_device *ptdev)
{
	struct panthor_vm *vm;
	...
	list_for_each_entry(vm, &ptdev->mmu->vm.list, node) {
		struct panthor_heap_pool *pool;
		u64 vm_id = panthor_vm_trace_id(vm);
		...
		panthor_heap_pool_dump_for_trace(pool, vm_id, 0);
