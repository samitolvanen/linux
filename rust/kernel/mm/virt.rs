...
impl VmaRef {
    ...
    /// must have either [`VM_PFNMAP`] or [`VM_MIXEDMAP`] set, but not both. `pfn` must reference
    /// a page the driver owns for the lifetime of the mapping.
    ///
    ...
    pub fn vmf_insert_pfn_prot(&self, address: usize, pfn: usize, pgprot: Pgprot) -> vm_fault_t {
        ...
        // caller holds at least the mmap read lock, which is what `vmf_insert_pfn_prot` needs.
        // The PFN and pgprot values are plain integers whose validity is the caller's.
...
///
/// Wraps the kernel's `pgprot_t`. The contents are architecture-defined and opaque. Drivers get
/// a `Pgprot` from [`VmaRef::vm_page_prot`], adjust it, and hand it back to the VM layer.
#[derive(Copy, Clone)]
#[repr(transparent)]
...
impl Pgprot {
    ...
    pub fn noncached(self) -> Self {
        // SAFETY: `pgprot_noncached` is a pure function of its argument with no
        // preconditions. Any `pgprot_t` value is a valid input.
