...
impl<T: drm::Driver> UnregisteredDevice<T> {
    ...
    unsafe extern "C" fn mmap_callback(
        ...
        let raw_file = unsafe { (*filp).private_data.cast::<bindings::drm_file>() };
        if raw_file.is_null() {
            return bindings::EINVAL as core::ffi::c_int;
        }
...
impl<T: drm::Driver, C: DeviceContext> Device<T, C> {
    ...
    ///
    /// The mount is torn down when the device is released, so there is no teardown call. This is
    /// a no-op returning `Ok` when `CONFIG_TRANSPARENT_HUGEPAGE` is disabled.
    #[inline]
...
// Dma-fence mirror of the `WorkItem<ID> for Device<T>` shim above. Forwards
// `Device<T>: DmaFenceWorkItem<ID>` to `T::Data` so a device handle can be
// enqueued onto a `DmaFenceWorkqueue`.
