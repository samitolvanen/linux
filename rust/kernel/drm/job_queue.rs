...
struct JobDependencies<T: QueueOps> {
    ...
    /// Pre-allocated uninitialized storage for the per-dep callback
...
struct JobEntry<T: QueueOps> {
    ...
    /// Pre-allocated uninitialized storage for the progress callback registered
    ...
    /// Pre-allocated uninitialized storage for the stage-wake callback
    /// registered in [`process_stage`](JobQueueInner::process_stage) on
    /// [`StageAdvance::WaitOn`]. Consumed once. Later `WaitOn`
...
impl<T: QueueOps> JobQueueInner<T> {
        ...
        loop {
            ...
            match advance {
                ...
                StageAdvance::WaitOn(fence) => {
                    ...
                    // The first WaitOn uses the prealloc slot. A re-registration
                    // falls back to GFP_NOWAIT, never GFP_KERNEL under the state mutex.
                ...
                StageAdvance::WaitFor(delay) => {
                    // SAFETY: The pending work item is an `Arc<Self>` and `self.wq` is an owning
                    // `Arc<DmaFenceWorkqueue>` field of `Self`, so the workqueue stays alive while
                    // the item is pending.
    ...
    fn process_deps(self: &Arc<Self>, entry_idx: u32) -> StageAdvance {
        loop {
            ...
            // Advance past an already-signaled dep, releasing any callback left
            // over from the previous one, without consuming a prealloc slot.
            ...
            // One prealloc slot per dep callback registration. prepare() sized
            // prealloc_cbs to dep_capacity, so GFP_NOWAIT is only a fallback
            // and stays out of direct reclaim.
            let uninit_box = {
                let mut guard = self.fifo.lock();
                guard
                    .get_mut(entry_idx as usize)
...
impl<T: QueueOps> PreparedJob<T> {
    ...
    ///
    /// The capacity must have been reserved by the `extra_dep_capacity`
    /// argument to [`JobQueue::prepare`]. The push allocates nothing, so it
    /// is safe inside a dma-fence signalling section. Returns `ENOSPC` if the
    /// caller asked for too few extra slots, and `EINVAL` if the job has
    ...
    /// Returns the reserved job, or `None` if it has already been
    /// committed or dropped.
    ///
    /// The driver may use this to populate interior-mutable fields of
    /// the job after prepare has fully succeeded, so a failure earlier in
    /// prepare leaves no partially initialized state behind.
    pub fn job(&self) -> Option<Arc<T::Job>> {
        let idx = self.xa_index.as_ref()?;
        let guard = self.inner.fifo.lock();
        let XaEntry::Reserved { job, .. } = guard.get(idx.index())? else {
            return None;
        };
        Some(job.clone())
    }
...
impl<T: QueueOps> JobQueue<T> {
    ...
    pub fn prepare(
        ...
        // One uninitialized callback slot per dep slot, so that
