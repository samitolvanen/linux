...
impl<T: QueueOps> JobQueueInner<T> {
    ...
    fn process_driver_stage(&self, stage: Arc<dyn StageOps<T>>, entry_idx: u32) -> StageAdvance {
        ...
        let (fence_ctx, fence_seqno) = trace_dbg::fence_key(&submit_fence);
        trace_dbg::fence_state(
            entry_idx,
            fence_ctx,
            fence_seqno,
            submit_fence.is_signaled(),
        );
        stage.process(&ctx)
    }
...
/// fences, and then cancels the stage timer synchronously. It must not
...
impl<T: QueueOps> PreparedJob<T> {
    ...
    /// prepare leaves no partially initialised state behind.
