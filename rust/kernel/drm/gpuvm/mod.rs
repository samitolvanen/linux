...
impl<T: DriverGpuVm> UniqueRefGpuVm<T> {
    ...
    pub fn find_first(&self, addr: u64, range: u64) -> Option<&GpuVa<T>> {
        ...
        // where `addr + range` overflows `u64` is one address stricter than
        // it needs, and keeps the FFI call free of wraparound.
        addr.checked_add(range)?;

        // SAFETY: `self.as_raw()` is a valid `drm_gpuvm`, and the
        ...
        // returned borrow. The filters above leave only VAs with an associated
        // `GpuVmBo<T>` on the GEM list, satisfying `GpuVa::from_raw`'s contract.
