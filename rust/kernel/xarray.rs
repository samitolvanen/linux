...
/// undo the reservation). Carries no `Drop` impl intentionally — callers are
...
impl<'a, T: ForeignOwnable> Guard<'a, T> {
    ...
    /// Cyclically allocates an index and marks the slot as reserved (no value stored yet).
    ///
    ...
    /// [`store_reserved`](Guard::store_reserved) or freed with [`release`](Guard::release).
    pub fn alloc_cyclic_reserve(
        ...
        // SAFETY: `self.xa.xa` is valid by the type invariant, the caller holds the lock,
        // and `XA_ZERO_ENTRY` is a valid XArray sentinel.
        let ret = unsafe {
            bindings::__xa_alloc_cyclic(
                self.xa.xa.get(),
                &mut id,
    ...
    /// Stores a value into a slot previously reserved by
    ...
    /// an existing `T` value.
    ///
    /// No GFP flags are needed: storing into a reserved slot walks existing
    /// `xa_node` structures created by `alloc_cyclic_reserve` without allocating
    ...
    ///
    /// Erases the `XA_ZERO_ENTRY` sentinel, making the slot available for future
    /// allocations.
    pub fn release(&mut self, index: ReservedIndex) {
        // SAFETY: `self.xa.xa` is valid by the type invariant and the caller holds
        // the lock. `Guard` holds `&mut self`, so no concurrent modification is
        // possible — a compare-exchange is unnecessary.
        unsafe {
            bindings::__xa_erase(self.xa.xa.get(), index.index());
        }
    }
}
