...
impl Devfreq {
    ...
    /// later [`resume_device`](Self::resume_device). The device stays
    /// suspended until the count returns to zero. An unbalanced resume drives
    /// the count negative and spuriously resumes a running device.
    #[inline]
...
/// # Safety
///
/// `NAME` must be a valid null-terminated C string naming a registered
/// kernel governor, and that governor must read its `data` argument as
/// `Self::Raw`.
...
impl<T: Callbacks> Registration<T> {
    ...
    pub fn new<G: Governor>(
        ...
        let devfreq_nn = match err_ptr.and_then(|p| NonNull::new(p).ok_or(EINVAL)) {
            Ok(nn) => nn,
            Err(e) => {
                // devfreq_add_device() failed, but the devfreq device may outlive
                // this return if its release is deferred. That release still uses
                // the profile, and it removes the PM QoS notifiers that can read
                // driver_data, so leak both `context_ptr` and the driver data.
                return Err(e);
            }
        };
