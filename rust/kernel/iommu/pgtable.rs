...
pub struct IoPageTable<'a, F: IoPageTableFmt, A = CoreAlloc> {
...
unsafe impl<F: IoPageTableFmt, A: Send + Sync> Send for IoPageTable<'_, F, A> {}
...
unsafe impl<F: IoPageTableFmt, A: Send + Sync> Sync for IoPageTable<'_, F, A> {}

impl<F: IoPageTableFmt, A: 'static> ForLt for IoPageTable<'static, F, A> {
    type Of<'a> = IoPageTable<'a, F, A>;
}

// SAFETY: `IoPageTable<'a, F, A>` is covariant over `'a` because it holds
// `PhantomData<&'a Device<Bound>>`, which is covariant.
unsafe impl<F: IoPageTableFmt, A: 'static> CovariantForLt for IoPageTable<'static, F, A> {}

/// A device-managed io page table.
///
/// See [`IoPageTable::new_devres`].
pub type DevresIoPageTable<F, A = CoreAlloc> = DevresLt<IoPageTable<'static, F, A>>;
...
impl<F: IoPageTableFmt, A: PageTableAlloc> IoPageTable<'static, F, A> {
    /// Create a new device-managed `IoPageTable` with a custom allocator.
    ///
    /// See [`IoPageTable::new_devres`] and [`IoPageTable::new_with_alloc`].
    #[inline]
    pub fn new_devres_with_alloc(
        dev: &Device<Bound>,
        config: Config,
        allocator: Arc<A>,
    ) -> Result<DevresIoPageTable<F, A>> {
        // SAFETY: The page table keeps `dev` for its DMA operations and stays valid until
        // `DevresLt` frees it at unbind, which happens before the device's DMA ops are torn down.
        unsafe {
            DevresLt::new(
                dev,
                IoPageTable::<F, A>::new_with_alloc(dev, config, allocator),
            )
        }
    }
}
...
impl<'a, F: IoPageTableFmt> IoPageTable<'a, F> {
    ...
    pub fn new(dev: &'a Device<Bound>, config: Config) -> Result<IoPageTable<'a, F>> {
        Self::create(dev, config, None)
    }
}

impl<'a, F: IoPageTableFmt, A: PageTableAlloc> IoPageTable<'a, F, A> {
    ...
    pub fn new_with_alloc(
        ...
        config: Config,
        allocator: Arc<A>,
    ) -> Result<IoPageTable<'a, F, A>> {
        Self::create(dev, config, Some(allocator))
    }

    fn create(dev: &'a Device<Bound>, config: Config, allocator: Option<Arc<A>>) -> Result<Self> {
...
impl<F: IoPageTableFmt, A> IoPageTable<'_, F, A> {
...
impl<F: IoPageTableFmt, A> Drop for IoPageTable<'_, F, A> {
    fn drop(&mut self) {
...
impl<A> IoPageTable<'_, ARM64LPAES1, A> {
