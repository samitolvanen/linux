...
static int process(struct state *state, struct cached_die *cache, const char *s)
{
	...
	return check(cache_add_string(cache, s));
}
...
static int __process_reserved_struct(struct state *state,
				     struct cached_die *cache, Dwarf_Die *die)
{
	Dwarf_Die type;
...
static int __process_reserved_union(struct state *state,
				    struct cached_die *cache, Dwarf_Die *die)
{
...
static int process_reserved(struct state *state, struct cached_die *cache,
			    Dwarf_Die *die)
{
	if (!stable)
		return NOT_RESERVED;
...
static int process_union_type(struct state *state, struct cached_die *cache,
			      Dwarf_Die *die)
{
